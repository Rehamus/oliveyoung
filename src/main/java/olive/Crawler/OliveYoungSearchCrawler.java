...
public class OliveYoungSearchCrawler {
    ...
    public static List<String> getProductUrls(String url) {
        ...
        try {
            ...
            while (hasNextPage) {
                ...
                List<WebElement> productLists = driver.findElements(By.cssSelector("#ajaxList"));

                // 각 ul 블록을 순차적으로 처리
                for (WebElement productList : productLists) {
                    // 각 ul 블록 내의 모든 li 태그를 가져옴 (data-index가 있는)
                    List<WebElement> productItems = productList.findElements(By.cssSelector(".li_result"));
                ...
                try {
                    ...
                    WebElement nextPageButton = driver.findElement(By.cssSelector(
                            "#Contents > div > div.pageing.new > a:nth-child(" + (currentPage + 1) + ")"));
                    System.out.printf(nextPageButton.getText());
